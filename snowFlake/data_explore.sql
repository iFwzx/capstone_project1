...
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select max(CS_SOLD_DATE_SK) from TPCDS.RAW.CATALOG_SALES); -- 2024-04-15

----- Web Sales
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select min(WS_SOLD_DATE_SK) from TPCDS.RAW.WEB_SALES); -- 2021-01-02
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select max(WS_SOLD_DATE_SK) from TPCDS.RAW.WEB_SALES); -- 2024-04-15

------ Inventory
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select min(INV_DATA_SK) from TPCDS.RAW.INVENTORY); -- 2021-01-01
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select max(INV_DATA_SK) from TPCDS.RAW.INVENTORY); -- 2024-04-09
...
INNER JOIN TPCDS.RAW.INVENTORY as inv
    ON date.d_date_sk=inv.inv_data_sk
    AND inv_item_sk=1
